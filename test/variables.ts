export const USER_EMAIL_PATIENT = "patient@test.com";
export const USER_EMAIL_PHYSICIAN = "physician@test.com";
export const USER_EMAIL_RADIOLOGIST = "radiologist@test.com";

export const USER_PW = "testtest123";
export const USER_WRONG_PW = "testtest1234";

export const USER_UID_PATIENT = "ecJD9MhYvoWRieaPnAL014G7Avf1";
export const USER_UID_PHYSICIAN = "q8ZY1MwztoTaGIKQCOrt9jQjMNi1";
export const USER_UID_RADIOLOGIST = "T3e7wxi3khgwiZGp7fXejUJe2Qg2";

export const AUTH_TOKEN = new Map([
  ["PATIENT", ""],
  ["PHYSICIAN", ""],
  ["RADIOLOGIST", ""],
]);

export const USER_IMAGES_API_KEYS = [
  "uid",
  "url",
  "createdAt",
  "first_name",
  "last_name",
  "uploadedBy_title",
  "uploadedBy_first_name",
  "uploadedBy_last_name",
  "authors",
];

export const USER_PATIENTS_API_KEYS = [
  "uid",
  "dob",
  "first_name",
  "last_name",
  "email",
  "profile_image_url",
  "images",
];

export const MEET_OUR_RADIOLOGISTS = [
  {
    uid: "464xIBql69bqrg8F1BhzHDHtHsw1",
    title: "Dr.",
    first_name: "Emily",
    last_name: "Johnson",
    profile_image_url: null,
    expertise: "Diagnostic",
  },
  {
    uid: "oNgXBgPnbAZymU51OyJ0CQ9GcFD2",
    title: "Dr.",
    first_name: "David",
    last_name: "Smith",
    profile_image_url: null,
    expertise: "Neuroradiology",
  },
  {
    uid: "xc3CMA0sMZNLRp5qDVH6UU3FQO83",
    title: "Dr.",
    first_name: "Robert",
    last_name: "Rivera",
    profile_image_url: null,
    expertise: "Abdominal",
  },
  {
    uid: "T3e7wxi3khgwiZGp7fXejUJe2Qg2",
    title: "Dr.",
    first_name: "John",
    last_name: "Radiologist",
    profile_image_url: null,
    expertise: "Neuroradiology",
  },
  {
    uid: "kMBvFu9pG2T36w8zqHf99rDiq992",
    title: "Dr.",
    first_name: "Laura",
    last_name: "Davis",
    profile_image_url: null,
    expertise: "Musculoskeletal",
  },
  {
    uid: "6npcE5FQFweAYGAxDlxSARwz1qn2",
    title: "Dr.",
    first_name: "Monica",
    last_name: "Kirk",
    profile_image_url: null,
    expertise: "Pediatric",
  },
];
